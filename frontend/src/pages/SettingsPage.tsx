...
function SettingsPage() {
  ...
  const handleEmailNotificationsChange = async () => {
    ...
    const nextValue = !user.emailNotifications;
    if (nextValue) {
      const confirmed = confirm(`Сайт хоче надсилати email-сповіщення на пошту ${user.email}. Увімкнути сповіщення?`);
      if (!confirmed) return;
    }

    setIsSavingEmailNotifications(true);
    try {
      await updateProfile({ emailNotifications: nextValue });
      success(nextValue ? "Email-сповіщення увімкнено" : "Email-сповіщення вимкнено");
    } catch (err: any) {
      error(err.message || "Не вдалося змінити email-сповіщення");
    } finally {
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl p-8 shadow-sm border border-gray-100 dark:border-slate-700 space-y-8">
        ...
        <div className="flex justify-between items-center">
          ...
          <label className="relative inline-flex items-center cursor-pointer">
            ...
            />
            <div className="w-11 h-6 bg-gray-300 dark:bg-slate-600 peer-disabled:opacity-60 peer-disabled:cursor-not-allowed peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-emerald-300 dark:peer-focus:ring-emerald-700 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-emerald-500 dark:peer-checked:bg-emerald-400"></div>
          </label>
        </div>
      </div>
