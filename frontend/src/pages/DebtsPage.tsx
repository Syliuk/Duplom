...
function DebtsPage() {
  const { debts, fetchDebts, addDebt, updateDebt, addPayment, deleteDebt } = useDebtStore();
  ...
  const handleSaveDebt = async (e: React.FormEvent) => {
    ...
    const actionKey = editingDebt ? `edit-debt-${editingDebt.id}` : "add-debt";
    ...
    const handlePayment = async (debtId: number) => {
      ...
      const remaining = debt ? Number(debt.amount) - Number(debt.currentAmount) : 0;
      if (!paymentAmount || Number(paymentAmount) <= 0) {
        error(t("debts.paymentRequired"));
        return;
      }
      if (Number(paymentAmount) > remaining) {
        error(t("debts.paymentTooLarge"));
        return;
      }
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-3xl p-8 w-full max-w-md border border-gray-200 dark:border-slate-700">
            <h2 className="text-2xl font-bold mb-6">{editingDebt ? t("debts.edit") : t("debts.new")}</h2>
            ...
            <form onSubmit={handleSaveDebt} className="space-y-5">
              ...
              <div className="flex gap-3 pt-4">
                ...
                <button
                  type="submit"
                  disabled={pendingActions["add-debt"] || Boolean(editingDebt && pendingActions[`edit-debt-${editingDebt.id}`])}
                  className="flex-1 bg-blue-600 text-white py-3 rounded-2xl font-semibold disabled:bg-blue-400 disabled:cursor-not-allowed"
                >
                  {editingDebt ? t("common.saveChanges") : t("common.add")}
                </button>
              </div>
            </form>
          </div>
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-3xl p-8 w-full max-w-sm border border-gray-200 dark:border-slate-700">
            ...
            <input
              ...
              step="0.01"
              max={(() => {
                const debt = debts.find((item) => item.id === paymentModal);
                return debt ? Number(debt.amount) - Number(debt.currentAmount) : undefined;
              })()}
              value={paymentAmount}
