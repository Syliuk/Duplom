...
import { Edit2, Plus, Trash2, Target, TrendingUp } from "lucide-react";
import type { Goal } from "../types/goal";

function GoalsPage() {
  const { goals, fetchGoals, addGoal, updateGoal, deleteGoal, addContribution } = useGoalStore();
  ...
  const handleSaveGoal = async () => {
    ...
    const actionKey = editingGoal ? `edit-goal-${editingGoal.id}` : "add-goal";
  ...
  const handleContribution = async (id: number | null) => {
    ...
    const remaining = goal ? Number(goal.targetAmount) - Number(goal.currentAmount) : 0;
    if (Number(contribAmount) <= 0) {
      error(t("common.amountPositive"));
      return;
    }
    if (Number(contribAmount) > remaining) {
      error(t("goals.contributionTooLarge"));
      return;
    }
              ...
              <div className="flex justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">{getCategoryLabel(goal.category, t)}</p>
                  </div>
                </div>
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-3xl p-8 w-full max-w-md border border-gray-200 dark:border-slate-700">
            <h2 className="text-2xl font-bold mb-6 text-gray-900 dark:text-white">
              {editingGoal ? t("goals.edit") : t("goals.newGoal")}
            </h2>
            ...
            <div className="space-y-5">
              ...
              <div className="pt-4 flex gap-3">
                ...
                <button 
                  ...
                  disabled={pendingActions["add-goal"] || Boolean(editingGoal && pendingActions[`edit-goal-${editingGoal.id}`])}
                ...
                >
                  {editingGoal ? t("common.saveChanges") : t("goals.createGoal")}
                </button>
              </div>
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-3xl p-8 w-full max-w-sm border border-gray-200 dark:border-slate-700">
            ...
            <input
              ...
              step="0.01"
              max={(() => {
                const goal = goals.find((item) => item.id === contributionModal);
                return goal ? Number(goal.targetAmount) - Number(goal.currentAmount) : undefined;
              })()}
              value={contribAmount}
