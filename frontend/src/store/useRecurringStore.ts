...
interface RecurringState {
  ...
  updateRecurring: (id: number, data: any) => Promise<void>;
...
export const useRecurringStore = create<RecurringState>((set) => ({
  ...
  updateRecurring: async (id, data) => {
    const updated = await api.recurring.update(id, data);
    set((state) => ({
      recurring: state.recurring.map(item => item.id === id ? updated : item),
    }));
  },
